function [nom_doc_input, nom_doc_output ] = lancer_techebichev_squelette_Test( matrix, n)
...
while i <= longueur
    ...
    nom_doc_input = strcat('Input_squelette_', matrix{i, 3}, '_Test.txt');
    Input_slh = fopen(nom_doc_input, 'w+');
        
        % fichier de sortie Silhouette
    nom_doc_output = strcat('Output_squelette_', matrix{i, 3}, '_Test.txt');
        ...
        for j = 1 : taille
            if Matrice_Moment_Squeletisation(i, j) == 0                
               ...
               % disp (Matrice_Moment_silhouette(i, j));
                fprintf(Input_slh, ch1, Matrice_Moment_Squeletisation(i, j));                
            end
        end
        ch = '\n';
